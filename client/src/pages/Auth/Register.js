        ...
        <div className="md:bg-white bg-black">
            <div className="bg-black min-h-screen max-w-md flex flex-col justify-around items-center mx-auto">
                ...
                <div className="bg-white px-3 py-5 w-11/12 rounded-lg">
                    ...
                    <p className="mt-2 text-sm">Please insert your credentials</p>
                    <form className="mt-5" onSubmit={handleSubmit} method="POST">
                        <div>
                            <label for="email" className="block text-sm">Email</label>
                            <input name="email" id="email" type="email" className="mt-2 p-3 block w-full border-2 border-slate-500 rounded-lg text-sm" placeholder="Input email" />
                        </div>
                        <div className="mt-3">
                        ...
                        <button className="rounded-lg bg-black text-white mt-4 h-12 w-full" disabled={isSubmited}>{isSubmited ? 'Please Wait...' : 'Register'}</button>
                    </form>
                    {invalid && 
                    ...
                    <p className="text-center text-xs mt-4">Already have an account ? <Link className="font-bold" to ='/login'>Sign In</Link></p>
                </div>
            </div>
        </div>
    )
